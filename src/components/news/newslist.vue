...
//                url: 'http://topTMZ.top:8080/tmz/yundisc/newslist.json',
              url0:'http://topTMZ.top:8080/tmz/yundisc/newslist.json',
...
//             var url='http://topTMZ.top:8080/tmz/yundisc/newsguoji.json';
      ...
      methods: {
          ...
          this.$http.post(url, {}, {emulateJSON: true})
            .then(
              (data) => {
                console.log(data);
                this.newslist1 = [];
                this.newslist1 = data.body.result.data.slice(0, 10);
              },
              (error) => {
                console.log(error);
              }
        ...
        onInfinite(done) {
          let vm = this;
          vm.$http.get(this.url0).then((data) => {
            vm.counter++;
            vm.pageEnd = vm.num * vm.counter;
            vm.pageStart = vm.pageEnd - vm.num;
            let arr = data.body.result.data;
            let i = vm.pageStart;
            let end = vm.pageEnd;
            for (; i < end; i++) {
              let obj = {};
              ...
              if ((i + 1) >= data.body.result.data.length) {
